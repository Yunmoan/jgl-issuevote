...
- 普通成员提交的议题先进入 `pending_review`，不出现在正式议题列表，也不能评论或投票。
- `issue_creator` 和管理员的提交直接发布；其他成员可在“预审”页面审批待审议题，但不能审批自己提交的议题。
- 审批通过后议题转为 `open`；驳回必须填写原因，作者编辑后会自动重新提交预审。
- 每次提交、通过、驳回和重新提交都会记录到审计日志。
