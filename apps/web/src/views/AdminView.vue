      ...
      <section class="admin-content">
        ...
        <n-card v-else-if="active === 'settings'" title="站点设置" size="large" class="settings-card">
          ...
          <section class="settings-section" aria-labelledby="site-settings-heading">
            ...
            <n-form-item label="已关闭议题自动归档"><n-input-number v-model:value="closedIssueArchiveAfterDays" :min="0" :max="3650" clearable><template #suffix>天后</template></n-input-number><n-text depth="3">默认 60 天；填 0 禁用。</n-text></n-form-item>
            ...
            <div class="settings-time-presets settings-span-full">
              ...
              <n-form-item label="即时投票"><n-input-number v-model:value="timePresets.voteInstantMinutes" :min="3" :max="43200"><template #suffix>分钟</template></n-input-number></n-form-item>
              <n-form-item label="短周期投票"><n-input-number v-model:value="timePresets.voteShortMinutes" :min="3" :max="43200"><template #suffix>分钟</template></n-input-number></n-form-item>
              <n-form-item label="长周期投票"><n-input-number v-model:value="timePresets.voteLongMinutes" :min="3" :max="43200"><template #suffix>分钟</template></n-input-number></n-form-item>
            </div>
...
import { NAlert, NButton, NCard, NCheckbox, NCheckboxGroup, NColorPicker, NDataTable, NDivider, NForm, NFormItem, NIcon, NInput, NInputNumber, NMenu, NModal, NSelect, NSpace, NSwitch, NTag, NText, useDialog, useMessage } from 'naive-ui';
...
function validTimePresets() { const discussionValues = [timePresets.discussionShortDays, timePresets.discussionLongDays]; const voteValues = [timePresets.voteInstantMinutes, timePresets.voteShortMinutes, timePresets.voteLongMinutes]; return discussionValues.every((value) => Number.isInteger(value) && value >= 1 && value <= 43200) && voteValues.every((value) => Number.isInteger(value) && value >= 3 && value <= 43200) && timePresets.discussionShortDays <= timePresets.discussionLongDays && timePresets.voteInstantMinutes <= timePresets.voteShortMinutes && timePresets.voteShortMinutes <= timePresets.voteLongMinutes; }
