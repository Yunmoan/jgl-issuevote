...
async function syncFeishuUserDepartments(row: any) { syncingFeishuUserId.value = row.id; try { const viewer = await apiPost<{ groups: string[] }>(`/admin/users/${row.id}/feishu-departments/sync`); row.groups = viewer.groups; message.success(`已同步 ${row.displayName} 的飞书部门`); } catch (error) { message.error(error instanceof Error ? error.message : '飞书部门同步失败'); } finally { syncingFeishuUserId.value = null; } }
