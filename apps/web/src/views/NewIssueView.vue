<template>
  <main class="content-wrap form-page">
    <div class="page-title">
      <div>
        <h1>{{ manualReviewRequired ? '提交议题' : '创建议题' }}</h1>
        <p class="page-subtitle">清晰说明议题，并设置讨论与表决的参与范围。</p>
      </div>
    </div>
    ...
    <n-alert v-if="manualReviewRequired" type="info" :bordered="false"
      style="margin-bottom: 8px;">提交后将进入预审，普通成员或更高权限的其他成员通过后才会公开。</n-alert>
    <n-alert v-else-if="issueReviewMode === 'ai'" type="info" :bordered="false"
      style="margin-bottom: 8px;">点击第一步“下一步”后将自动进行 AI 预审，包括相似议题、法律法规风险和站点自定义条件检查。</n-alert>
    <n-form ref="formRef" :model="form" :rules="rules" label-placement="top" size="large">
      <n-card v-show="currentStep === 1" title="基本信息" size="large">
        ...
        <n-form-item label="分类"><n-select v-model:value="form.labelIds" multiple :options="labelOptions"
            :loading="loadingOptions" :disabled="!formOptionsReady" placeholder="选择议题分类" /></n-form-item>
      ...
      <n-alert v-if="optionsError" type="error" :bordered="false" title="无法读取创建议题所需的配置">
        <n-space align="center" :size="8"><span>{{ optionsError }}</span><n-button text type="primary"
            :loading="loadingOptions" @click="loadFormOptions">重试</n-button></n-space>
      </n-alert>
      ...
      <n-card v-show="currentStep === 2" title="参与范围" size="large" class="form-card">
        <n-grid :cols="2" :x-gap="20" :y-gap="4" responsive="screen" item-responsive>
          <n-gi span="2 720:1"><n-form-item label="可见范围"><n-select v-model:value="form.visibility"
                :options="visibilityOptions" /></n-form-item></n-gi>
          <n-gi span="2 720:1"><n-form-item label="查看权限组"><n-select v-model:value="form.viewGroupKeys" multiple
          ...
          <n-gi span="2 720:1"><n-form-item label="每人最多发表意见次数"><n-input-number v-model:value="form.maxCommentsPerUser"
                :min="1" :max="100" style="width: 100%"><template
                  #suffix>次</template></n-input-number></n-form-item></n-gi>
        </n-grid>
        <n-space vertical :size="8"><n-checkbox v-model:checked="form.commentAnonymous">意见与回复匿名展示</n-checkbox><n-text
            depth="3">不设置意见公布时间时，符合查看权限的用户会立即看到新意见。</n-text></n-space>
      </n-card>

      <n-card v-show="currentStep === 3" title="投票规则" size="large" class="form-card">
        <n-space vertical :size="16">
          <n-checkbox v-model:checked="form.votingEnabled" :disabled="form.issueType === 'cycle'">启用投票器</n-checkbox>
          ...
          <n-grid :cols="2" :x-gap="20" :y-gap="4" responsive="screen" item-responsive>
            ...
            <n-gi v-else span="2"><n-form-item label="投票时间"><n-radio-group v-model:value="voteTimeMode"><n-space
                    :size="12" :wrap="true"><n-radio value="manual">手动开启</n-radio><n-radio value="instant">即时投票（{{
                      ...
                      value="specific">手动设定时间</n-radio></n-space></n-radio-group></n-form-item></n-gi>
            <template v-if="form.issueType === 'custom' && voteTimeMode !== 'manual' && voteTimeMode !== 'specific'">
              <n-gi span="2 720:1"><n-form-item label="多久后开始投票"><n-input-number v-model:value="voteStartDelayMinutes"
                    :min="0" :max="43200" style="width: 100%"><template
                      #suffix>分钟后</template></n-input-number></n-form-item></n-gi>
              <n-gi v-if="voteTimeMode === 'minutes'" span="2 720:1"><n-form-item label="投票持续时间"><n-input-number
                    v-model:value="voteDurationMinutes" :min="MIN_VOTE_DURATION_MINUTES" :max="43200"
                    style="width: 100%"><template #suffix>分钟</template></n-input-number></n-form-item></n-gi>
              <n-gi span="2"><n-form-item label="计划"><n-text>{{ voteStartsAt ? `${formatTime(voteStartsAt)} 至
                    ${formatTime(voteEndsAt)}` : '未设置' }}</n-text></n-form-item></n-gi>
            </template>
            <template v-else-if="form.issueType === 'custom' && voteTimeMode === 'specific'">
              <n-gi span="2 720:1"><n-form-item label="自动投票开始"><n-date-picker v-model:value="voteStartsAt"
                    type="datetime" clearable style="width: 100%"
                    :is-date-disabled="disableFutureDate" /></n-form-item></n-gi>
              <n-gi span="2 720:1"><n-form-item label="自动投票结束"><n-date-picker v-model:value="voteEndsAt" type="datetime"
                    clearable style="width: 100%" :is-date-disabled="disableVoteEndDate" /></n-form-item></n-gi>
            </template>
            <n-gi v-else-if="form.issueType === 'custom'" span="2"><n-form-item
            ...
            <n-gi v-if="form.issueType !== 'election'" span="2">
              <div class="vote-rule-pair"><n-form-item label="基准人数（可选）" :show-feedback="false">
                  <div class="quorum-field"><n-input-number v-model:value="form.quorumCount" :min="0"
                      clearable><template #suffix>人</template></n-input-number><n-text depth="3"
      ...
      <n-card size="small" class="form-footer">
        <n-space justify="end">
          ...
          <n-button v-else @click="currentStep -= 1"><template #icon><n-icon>
                <ArrowBackOutline />
              </n-icon></template>上一步</n-button>
          <n-button v-if="currentStep < 3" type="primary" :loading="currentStep === 1 && aiReviewing"
            @click="nextStep">下一步<template #icon><n-icon>
                <ArrowForwardOutline />
              </n-icon></template></n-button>
          <n-button v-else type="primary" :loading="submitting" @click="submit"><template #icon><n-icon>
                <AddCircleOutline />
              </n-icon></template>{{ manualReviewRequired ? '提交预审' : '发布议题' }}</n-button>
        </n-space>
      </n-card>
    </n-form>
    <n-modal v-model:show="showSimilarIssueDialog" preset="card" title="发现相似议题"
      :style="{ width: 'min(620px, calc(100vw - 24px))' }" :bordered="false">
      <n-space vertical size="large"><n-text>系统找到以下最多 5 个相似议题。确认后仍可继续创建当前议题。</n-text><n-space vertical
          :size="8"><n-button v-for="issue in aiReview?.similarIssues || []" :key="issue.number" secondary
            @click="router.push(`/issues/${issue.number}`)">#{{ issue.number }} {{ issue.title
            }}</n-button></n-space></n-space>
      <template #footer><n-space justify="end"><n-button
            @click="showSimilarIssueDialog = false">返回修改</n-button><n-button type="primary"
            @click="continueAfterSimilarIssues">继续创建</n-button></n-space></template>
    </n-modal>
  </main>
</template>
...
async function loadFormOptions() {
  ...
  optionsError.value = '';
  try {
    const [groups, labels, config] = await Promise.all([apiGet<Array<{ groupKey: string; name: string }>>('/permission-groups'), apiGet<Array<{ id: number; name: string }>>('/labels'), apiGet<{ defaultIssueVisibility: 'public' | 'login' | 'groups'; issueReviewMode?: 'disabled' | 'manual' | 'ai'; timePresets?: typeof timePresets.value; currentCycle?: { weekLabel: string; startAt: string; endAt: string; label: string } }>('/site-config')]);
    ...
    currentCycle.value = config.currentCycle || null;
...
watch(() => form.issueType, (issueType) => {
...
watch(() => [form.issueType, form.electionScope, form.electionStartMode, form.electionDurationPreset, voteStartsAt.value], () => {
  if (form.issueType === 'election') {
    if (form.electionScope === 'all') form.voteGroupKeys = [];
    if (form.electionStartMode === 'manual') {
      voteStartsAt.value = null;
      voteEndsAt.value = null;
    } else if (voteStartsAt.value) {
      const duration = form.electionDurationPreset === 'instant' ? 10 : 24 * 60;
      voteEndsAt.value = voteStartsAt.value + duration * 60 * 1000;
    }
  }
}, { immediate: true });
